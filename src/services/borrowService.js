...
export async function getEmergencyRequests() { const { data, error } = await requireSupabase().from("emergency_requests").select("id,item_title,note,expires_at,profiles!emergency_requests_requester_id_fkey(full_name)").gt("expires_at", new Date().toISOString()).order("created_at", { ascending: false }); fail(error); return data.map((x) => ({ id: x.id, itemTitle: x.item_title, note: x.note, requesterName: x.profiles?.full_name || "Community member", expiresAt: new Date(x.expires_at).getTime() })); }
